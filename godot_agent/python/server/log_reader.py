...
def _file_of(err):
    """Файл, в котором произошла ошибка — из строки location вида
    «at: func (res://x.gd:12)». Нужен для группировки: ошибки в одном файле
    почти всегда правятся одним патчем."""
    loc = err.get("location") or ""
    m = re.search(r"\((res://[^:)]+):\d+\)", loc)
    if m:
        return m.group(1)
    m = re.search(r"(res://[^\s:)]+)", loc)
...
def format_report(report):
    ...
    ПРО СТРАТЕГИЮ ИСПРАВЛЕНИЯ. Раньше здесь стояло «исправляй ПО ОДНОЙ,
    начиная с первой». Причина была верной — сообщения 2..N часто следствие
    первой ошибки, — но цена высокая: пять ошибок означали пять обращений к
    модели. Для работы по ключу API это пять оплаченных запросов вместо
    одного, а на бесплатных тарифах первым упирается именно лимит числа
    запросов.

    Теперь инструкция другая: исправить ВСЕ НЕЗАВИСИМЫЕ ошибки одним планом,
    но не гнаться за следствиями. Дополнительно ошибки сгруппированы по
    файлам — так модели видно, что несколько сообщений относятся к одному
    месту и правятся одним патчем.
    """
    fence = "`" * 3
    n = len(report["errors"])
    parts = [
        "[Система: Ошибки последнего запуска игры (лог от %s). Уникальных ошибок: %d." % (report["log_time"], n),
        "Исправь ВСЕ НЕЗАВИСИМЫЕ ошибки ОДНИМ ответом: правки в разных файлах — "
        "через action=plan, несколько правок в одном файле — по возможности одним patch_file. "
        "НО: если часть сообщений вызвана ОДНОЙ причиной (например, не зарегистрирована "
        "автозагрузка, не найден узел, опечатка в имени класса) — исправь ТОЛЬКО причину и "
        "перечисли, какие ошибки должны исчезнуть вместе с ней. Не заглушай следствия "
        "проверками на null и не правь то, в чём не уверен: лучше оставить ошибку и сказать "
        "об этом, чем внести неверную правку.",
    ]
    # Ошибки одного файла идут подряд, с подписью файла.
    by_file = {}
    order = []
    ...
    for key in order:
        ...
        if key and len(group) > 1:
            parts.append("--- Файл %s: %d ошибок(и) ниже относятся к нему ---"
                         % (key, len(group)))
        for e in group:
            idx += 1
