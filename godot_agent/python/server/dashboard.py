...
# RLock, а не Lock: строка журнала может появиться в момент, когда замок уже
# держит этот же поток (вложенный print из кода, который вызван при подготовке
# строки). С обычным Lock это давало ТИХИЙ ДЕДЛОК — сервер вставал молча, без
# исключения, на первой же такой строке.
_lock = threading.RLock()

# Защита от повторного входа в маскирование. Нужна не теоретически: api_keys
# при испорченном файле настроек сам печатает предупреждение, а печать снова
# идёт через маскирование — получалась бы бесконечная рекурсия и падение
# сервера на первой же строке журнала. Флаг на поток, а не глобальный: иначе
# строка другого потока могла бы проскочить в журнал без маскирования.
_guard = threading.local()


def _redact(line):
    """Убирает секреты из строки ПЕРЕД попаданием в журнал.

    Журнал отдаётся HTTP-страницей /dashboard, то есть виден по сети и на
    любой записи экрана. Полагаться на аккуратность каждого print в проекте
    нельзя — один случайный вывод заголовков запроса раскрыл бы ключ API.
    Барьер стоит здесь, на выходе, а не у каждого вызывающего.

    В консоль строка идёт КАК ЕСТЬ: там она нужна для отладки, и туда по сети
    никто не смотрит. Ошибка маскирования не должна ронять вывод сервера,
    поэтому любое исключение означает «отдать строку без изменений».
    """
    if getattr(_guard, "busy", False):
        # Маскирование напечатало что-то само. Отдаём как есть: строка эта
        # служебная («файл настроек не читается»), секретов в ней нет.
        return line
    _guard.busy = True
    try:
        import api_keys
        return api_keys.redact(line)
    except Exception:
        return line
    finally:
        _guard.busy = False
    ...
    def write(self, s):
        ...
        try:
            ...
            while "\n" in self._part:
                ...
                line = line.rstrip("\r")
                if line.strip():
                    # Маскирование ДО взятия замка: оно читает настройки и в
                    # редких случаях печатает сообщение само, а печать снова
                    # пришла бы сюда и попыталась взять тот же замок.
                    safe = _redact(line)
                    with _lock:
                        _lines.append("%s %s" % (time.strftime("%H:%M:%S"), safe))
        except Exception:
            pass
