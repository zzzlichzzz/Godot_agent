...
def status(project_root, addon_dir=None):
    ...
    status() не запускает обучение.
    Он читает состояние mini-lich и может инициализировать или мигрировать
    локальное хранилище. Локальная миграция хранилища допускается.
    ...
    server.exe) и пользователь не переключил галочку вручную в этой сессии —
    обучение продолжается не автоматически, а только по явному действию:
    Запуск обучения — только явное действие (/minilich/set с enabled=true).
    v106: прежнее «самолечение» статуса удалено по репорту — оно стартовало
    обучение из читающих эндпоинтов: POST /minilich/status (панель шлёт его
    при КАЖДОМ открытии настроек, а кнопка «Обновить справочник API» живёт в
    том же диалоге) и GET /dashboard/data (страница дашборда). Пользователь
    видел, что после нажатия кнопки в логах сервера начинается обучение,
    хотя секция mini-lich скрыта и галочку он не трогал.
    """
    if addon_dir:
        try:
            ml_data.set_storage_base(addon_dir, project_root)
    ...
    try:
        from . import ml_train
        # v106: status() не запускает обучение. Здесь раньше вызывался
        # start_training() при enabled=True; из-за этого обучение
        # незаметно включали обычные опросы статуса (настройки/
        # дашборд). Локальная миграция хранилища по-прежнему допускается;
        # запуск — только /minilich/set.
