...
СЕКРЕТОВ ЗДЕСЬ НЕТ: только идентификаторы моделей, признак бесплатности, то,
что провайдер сам присылает о модели (цена, окно контекста, предел ответа), и
время получения списка. Файл можно показывать и пересылать целиком.
"""
import json
import os
import time
...
# ЧТО ЕЩЁ, КРОМЕ id И free, ХРАНИТ ЗАПИСЬ МОДЕЛИ. Только то, что присылает САМ
# ПРОВАЙДЕР в своём /models: цена, окно контекста, предел ответа, поддержка
# родного function calling. У OpenRouter это есть у всех 416 записей (замерено),
# у Opencode Zen в записи вообще только id — там будет пусто, и это нормально.
#
# ДОПОЛНЕНИЯ ИЗ КАТАЛОГА models.dev ЗДЕСЬ НЕ ХРАНЯТСЯ, И ЭТО ГЛАВНОЕ ПРАВИЛО
# ФАЙЛА. Причина не в размере, а в честности подписи: панель показывает цену с
# указанием источника («по ответу провайдера» против «по каталогу models.dev»),
# а после записи на диск и чтения обратно отличить одно от другого стало бы
# невозможно — сложенное каталогом поле выглядело бы как присланное
# провайдером. Поэтому кэш держит ПЕРВИЧНУЮ правду, а каталог прикладывается на
# выходе (chat_routes._enrich) и сам сообщает, что дописал (from_catalog).
# Побочная выгода: обновление каталога действует сразу, не дожидаясь нового
# опроса провайдеров.
_LIVE_NUM_FIELDS = ("cost_in", "cost_out")
# created — время появления модели из ответа провайдера (формат OpenAI, секунды).
# Хранится вместе с остальным живым знанием: по нему список сортируется так,
# чтобы новые модели были выше старых.
_LIVE_INT_FIELDS = ("context", "max_output", "created")
_LIVE_BOOL_FIELDS = ("tool_call",)
# Поля каталога, которые сюда попасть НЕ должны. Перечислены явно, чтобы
# случайно записанная обогащённая запись не унесла их на диск молча.
_CATALOG_ONLY = ("catalog_free", "status", "deprecated", "from_catalog")
...
def _clean_records(records):
    ...
    Пропускаются ТОЛЬКО поля из живого ответа провайдера (см. _LIVE_* выше).
    Дополнения каталога отбрасываются намеренно: попав на диск, они стали бы
    неотличимы от присланных провайдером, и подпись об источнике начала бы
    врать. Отсутствующее поле не выдумывается: нет цены — значит про цену этой
    модели ничего не известно, а ноль вместо неё означал бы «бесплатная».
    """
    out = []
    seen = set()
    for rec in list(records or [])[:MAX_PER_PROVIDER * 2]:
        ...
        if isinstance(rec, dict):
            ...
            for f in _LIVE_NUM_FIELDS:
                if isinstance(rec.get(f), (int, float)) and not isinstance(rec.get(f), bool):
                    extra[f] = float(rec[f])
            for f in _LIVE_INT_FIELDS:
                if isinstance(rec.get(f), (int, float)) and not isinstance(rec.get(f), bool):
                    val = int(rec[f])
                    if val > 0:
                        extra[f] = val
            for f in _LIVE_BOOL_FIELDS:
