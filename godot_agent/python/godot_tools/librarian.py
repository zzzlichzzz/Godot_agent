# -*- coding: utf-8 -*-
"""v105: «Библиотекарь» (action=ask_librarian) — единая точка выдачи
данных о проекте для модели. Выполняется АВТОМАТИЧЕСКИ, без подтверждения
пользователем: исходные файлы проекта не меняются; локальный индекс и
librarian_log могут обновляться в .agent_history (как search_project, но
