...
# ГЛАВНОЕ СВОЙСТВО РОТАЦИИ: меняется ТОЛЬКО ключ.
#
# Ключ — это про деньги и квоту, модель — про то, кто отвечает. Если при смене
# ключа подменить ещё и модель (например, уйти на бесплатную «чтобы точно
# сработало»), человек получит ответ от другой нейросети, не узнав об этом, а
# дальше в переписке будут смешаны два разных стиля и две разные точности
# соблюдения формата действий — разобраться в такой истории потом невозможно.
# Проверка ловит именно это: тела ОБОИХ запросов должны называть одну модель —
# ту, что закреплена за чатом.
check(u"при смене ключа модель НЕ подменяется",
      [r.get("model") for r in ALL_REQUESTS] == [ROT_REC["model"]] * 2)
check(u"и провайдер остался тот же — ротация идёт внутри одного сервиса",
      [u.split("-v1-")[0] for u in AUTH_SEEN] == ["Bearer sk-or"] * 2)
